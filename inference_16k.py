...
if __name__ == '__main__':
    ...
    tokenizer = AudioTokenizerExt(sample_rate=params.sample_rate, device="cuda")
    ...
    with torch.no_grad():
        for i, text in enumerate(texts):
            ...
            # Y_t
            code_enc = tokenizer.encode_emb(y_enc)
            audio_enc = tokenizer.decode(code_enc).squeeze()
            print(audio_enc.shape)
            # Y_0
            code_dec = tokenizer.encode_emb(y_dec)
            audio_dec = tokenizer.decode(code_dec).squeeze()
            print(audio_dec.shape)

            # audio = (vocoder.forward(y_dec).cpu().squeeze().clamp(-1, 1).numpy() * 32768).astype(np.int16)
            
            write(f'./out/sample_{i}_enc.wav', 16000, audio_enc.cpu().numpy())
            write(f'./out/sample_{i}_dec.wav', 16000, audio_dec.cpu().numpy())
